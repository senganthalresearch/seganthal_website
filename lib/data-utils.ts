...
export function quarterReturns(points:{date:string;close:number}[],now=new Date(),includeCurrent=false){
 const current=now.getUTCFullYear()*4+Math.floor(now.getUTCMonth()/3);
 const ends=new Map<number,number>();for(const p of [...points].sort((a,b)=>a.date.localeCompare(b.date))){const d=new Date(p.date);if(!Number.isFinite(d.getTime())||!Number.isFinite(p.close)||p.close<=0)continue;const q=d.getUTCFullYear()*4+Math.floor(d.getUTCMonth()/3);if(includeCurrent?q<=current:q<current)ends.set(q,p.close)}
 const maxQ=includeCurrent?current:current-1;
 return Array.from({length:8},(_,i)=>{const q=maxQ-7+i,previous=ends.get(q-1),close=ends.get(q);return {quarter:`${Math.floor(q/4)} Q${q%4+1}${includeCurrent&&q===current?' (QTD)':''}`,value:previous&&close?((close/previous)-1)*100:null}});
}
