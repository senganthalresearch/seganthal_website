...
export const GET=api(async request=>{const member=await requirePermission('analyze');await limited(member,'stakes-read',40);const params=new URL(request.url).searchParams;if(params.has('symbol'))return stake(symbolSchema.parse(params.get('symbol')));return {scan:await(await db()).collection<Scan>('stakeScans').findOne({owner:member.email},{projection:{_id:0,owner:0}})}});
export const POST=api(async request=>{
 checkOrigin(request);const member=await requirePermission('analyze');await limited(member,'stakes-scan',40);
